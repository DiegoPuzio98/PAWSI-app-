...
interface Highlight { id: string; post_id: string; post_type: 'lost'|'reported'|'adoption'|'classified'; }

interface BasePost { id: string; title: string; images?: string[]; created_at: string; location_text?: string; status: string; }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (rpIds.length) queries.push(supabase.from('reported_posts').select('*').in('id', rpIds));
        if (lpIds.length) queries.push(supabase.from('lost_posts').select('*').in('id', lpIds));
        ...
        setPosts(collected.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()));
      } catch (e: any) {
        console.error(e);
              ...
              <Card key={`${post.post_type}-${post.id}`} className="overflow-hidden">
                {post.images?.[0] && (
                  <div className="aspect-video bg-muted">
                    <img
                      src={post.images[0].startsWith('http') ? post.images[0] : `https://jwvcgawjkltegcnyyryo.supabase.co/storage/v1/object/public/posts/${post.images[0]}`}
                      alt={post.title}
                ...
                <CardContent className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <Badge className={post.status === 'resolved' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}>
                  ...
                  <PostActions
                    ...
                    isHighlighted={true}
                    onHighlightChange={(h) => {
                      if (!h) setPosts(prev => prev.filter(p => !(p.id === post.id && p.post_type === post.post_type)));
                    }}
                  />
                </CardContent>
              </Card>
